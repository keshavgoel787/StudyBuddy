...
from app.services.google_calendar import (
    get_todays_events, get_week_events, create_calendar_event,
...
async def get_week_calendar_events(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    Get all calendar events for the current week (Sunday to Saturday).
    Returns events from user's Google Calendar without AI processing.
    """
    try:
        # Get user's validated Google tokens
        user_token = _get_user_calendar_token(str(current_user.id), db)
        ...
        # Return events in simple format
        return {
            "events": [
                {
                    "id": event.id,
        ...
        import traceback
        print(f"ERROR in get_week_calendar_events: {str(e)}")
        print(f"Traceback: {traceback.format_exc()}")
...
async def get_bus_schedule(
    ...
    """
    Get available bus times for today, filtered by user's calendar events.
    Returns outbound (Main & Murray → UDC) and inbound (UDC → Main & Murray) schedules.
    Only shows buses that make sense based on campus event times.
    """
    try:
        today = date.today()

        # Get user's Google Calendar events for today
        user_token = _get_user_calendar_token(str(current_user.id), db)
